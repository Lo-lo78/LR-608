...
int main()
{
    ...
    const double cowFactory[7][11]{{8,.32,.45,.55,1,.4,.35,.3,10,.25,1},{6,.5,2,.5,0,.5,.5,0,0,0,1},{6,.1,2,.5,10,5,.5,0,0,0,1},{9,0,2.3,0,1,.4,.35,.3,5,.25,1},{6,.5,2,.5,0,.5,.5,0,0,0,1},{5,.8,2.75,1,4.5,5,.7,.3,3.5,.18,.16},{5,.8,2.75,1,4.5,5,.7,.3,3.5,.18,.16}};
    for(int engine=0;engine<7;++engine){lr608::CowbellParameters cp;std::copy(std::begin(cowFactory[engine]),std::end(cowFactory[engine]),cp.v.begin());lr608::CowbellVoice cow;cow.prepare(48000);cow.trigger(engine,127,cp);double e=0;for(int i=0;i<240000;++i){const auto s=cow.render(cp);if(!std::isfinite(s)){std::cerr<<"Non-finite Cowbell "<<engine<<'\n';return 1;}e+=s*s;}if(e<.000001){std::cerr<<"Silent Cowbell "<<engine<<'\n';return 1;}std::cout<<"Cowbell engine "<<engine<<" energy="<<e<<'\n';}
