...
inline constexpr int slotParameterValueCount=348;
...
inline constexpr int slotDelayWetParameterIndex=276;
inline constexpr int slotDelayDivisionParameterIndex=277;
inline constexpr int slotDelayFeedbackParameterIndex=278;
inline constexpr int slotDelayGlideParameterIndex=279;
inline constexpr int slotDelayFilterParameterIndex=280;
...
inline constexpr int slotDegradeAmountParameterIndex=285;
inline constexpr int slotDegradeBitsParameterIndex=286;
inline constexpr int slotDegradeHoldParameterIndex=287;
inline constexpr int slotDegradeJitterParameterIndex=288;
inline constexpr int slotFilterEnvelopeDepthParameterIndex=289;
inline constexpr int slotFilterEnvelopeDecayParameterIndex=290;
...
{
 int current=engine;
 if(engine<=6)current=engine;
 else if(engine<=14)current=engine+1;
 else if(engine<=20)current=engine+2;
 else if(engine<=24)current=engine+3;
 else current=engine+5;
 // Architecture v9 inserted Timbales Sample Captured before the Zap family.
 return current>=78?current+1:current;
}
