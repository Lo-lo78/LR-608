...
int main()
{
    ...
    for(int engine=0;engine<8;++engine){lr608::CowbellParameters cp;std::copy(std::begin(cowFactory[engine]),std::end(cowFactory[engine]),cp.v.begin());static constexpr double capturedDefaults[57]{4.0, 100.0, 100.0, 0.0, 100.0, 6.5, 3.8, 100.0, 3675.0, 18.0, 55.0, 0.0, 60.0, 193.0, 388.0, 560.0, 732.0, 889.0, 1150.0, 1490.0, 1990.0, 2150.0, 2400.0, 10.0, 42.0, 58.0, 83.0, 34.0, 25.0, 16.0, 10.0, 8.5, 7.0, 90.0, 115.0, 105.0, 185.0, 120.0, 82.0, 60.0, 43.0, 38.0, 30.0, 24.0, 22.0, 7.5, 30.0, 70.0, 135.0, 72.0, 35.0, 30.0, 22.0, 18.0, 20.0, 65.0, 1.2};std::copy(std::begin(capturedDefaults),std::end(capturedDefaults),cp.captured.begin());lr608::CowbellVoice cow;cow.prepare(48000);cow.trigger(engine,127,cp);double e=0;for(int i=0;i<240000;++i){const auto s=cow.render(cp);if(!std::isfinite(s)){std::cerr<<"Non-finite Cowbell "<<engine<<'\n';return 1;}e+=s*s;}if(e<.000001){std::cerr<<"Silent Cowbell "<<engine<<'\n';return 1;}std::cout<<"Cowbell engine "<<engine<<" energy="<<e<<'\n';}
    ...
    for(int engine=0;engine<8;++engine){lr608::CowbellParameters q;std::copy(std::begin(cowFactory[engine]),std::end(cowFactory[engine]),q.v.begin());static constexpr double capturedDefaults2[57]{4.0, 100.0, 100.0, 0.0, 100.0, 6.5, 3.8, 100.0, 3675.0, 18.0, 55.0, 0.0, 60.0, 193.0, 388.0, 560.0, 732.0, 889.0, 1150.0, 1490.0, 1990.0, 2150.0, 2400.0, 10.0, 42.0, 58.0, 83.0, 34.0, 25.0, 16.0, 10.0, 8.5, 7.0, 90.0, 115.0, 105.0, 185.0, 120.0, 82.0, 60.0, 43.0, 38.0, 30.0, 24.0, 22.0, 7.5, 30.0, 70.0, 135.0, 72.0, 35.0, 30.0, 22.0, 18.0, 20.0, 65.0, 1.2};std::copy(std::begin(capturedDefaults2),std::end(capturedDefaults2),q.captured.begin());auto v=std::make_unique<lr608::CowbellVoice>();v->prepare(48000);v->trigger(engine,127,q);double last=0;for(int i=0;i<terminationLimit&&v->isActive();++i)last=v->render(q);if(v->isActive())std::cerr<<"Cowbell residual="<<last<<'\n';if(!requireStopped(v->isActive(),"Cowbell",engine))return 1;}
